...
def build_model(file_name):
	...
	for x in xrange(n_c):
		plt.scatter(data[:,0][output==x],data[:,1][output==x],marker=m[x],s=45,label='Class %s' %x)
...
def plot_traj_image(filename,data):
	...
	for l in set(output):
		...
		for element in index_of_data:
			...
			if l in done:
				plt.scatter(x,y,marker=m[l],s=50)
			else:
				done.append(l)
				plt.scatter(x,y,marker=m[l],s=50,label='Class %s' %l)
	plt.legend(loc='upper left')
	plt.show()
